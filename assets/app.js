...
function state(){return {q:$('q').value,topic,sphere:$('sphere').value,status:$('status').value,order:$('order').value};}
function readURL(){const p=new URLSearchParams(location.search);$('q').value=p.get('q')||'';topic=p.get('topic')||'';for(const k of ['sphere','status','order']){const v=p.get(k)|| (k==='order'?'desc':'');$(k).value=[...$(k).options].some(o=>o.value===v)?v:(k==='order'?'desc':'');}if(!events.some(e=>e.topic===topic))topic='';$('advanced').open=!!($('sphere').value||$('status').value||$('order').value==='asc');}
...
function card(e){const c=el('article',null,'card');c.id=e.id;
 ...
 const comparison=el('div',null,'comparison');for(const [label,text] of [['ANTES',e.before],['DEPOIS',e.after]]){const d=el('div');d.append(el('strong',label),el('p',text));comparison.append(d);}c.append(comparison,paragraph('Para quem:',e.affected),paragraph('Quando:',e.dateNote),el('p',e.spheres.join(' · ')+' · '+e.instrument,'meta'));
...
function render(updateURL=true){
 ...
 $('result-status').textContent=filtered.length+' registro(s) nesta seleção • '+($('order').value==='asc'?'mais antigos primeiro':'mais recentes primeiro');
 ...
 let lastYear,grid;
 for(const e of filtered){if(e.year!==lastYear){const group=el('details',null,'year');group.open=true;group.append(el('summary',String(e.year)));grid=el('div',null,'cards');group.append(grid);out.append(group);lastYear=e.year;}grid.append(card(e));}
 if(updateURL)syncURL();
}
function reset(){for(const k of ['q','sphere','status'])$(k).value='';$('order').value='desc';topic='';render();}
$('filters').onsubmit=e=>e.preventDefault();$('q').oninput=()=>render();for(const k of ['sphere','status','order'])$(k).onchange=()=>render();$('reset').onclick=reset;
...
async function loadEvents(){
 ...
 try{const r=await fetch('data/events.json');if(!r.ok)throw Error();const data=await r.json();events=validateEvents(data);$('coverage').textContent=events.length+' registros conferidos • Revisão: '+date(data.reviewedAt)+' • '+data.coverage;
 ...
 const id=new URLSearchParams(location.search).get('registro');if(id && events.some(e=>e.id===id)){reset();}const hash=location.hash.slice(1);if(events.some(e=>e.id===hash))$(hash)?.scrollIntoView();
