...
function state(){return {q:$('q').value,topic,sphere:$('sphere').value,status:$('status').value,order:$('order').value,year:$('year').value,impact:$('impact').value,president:$('president').value,authority:$('authority').value};}
function readURL(){const p=new URLSearchParams(location.search);$('q').value=p.get('q')||'';topic=p.get('topic')||'';for(const k of ['sphere','status','order','year','impact','president','authority']){const v=p.get(k)|| (k==='order'?'desc':'');$(k).value=[...$(k).options].some(o=>o.value===v)?v:(k==='order'?'desc':'');}if(!events.some(e=>e.topic===topic))topic='';$('advanced').open=!!($('sphere').value||$('status').value||$('year').value||$('impact').value||$('president').value||$('authority').value||$('order').value==='asc');}
...
function card(e){const c=el('article',null,'card');c.id=e.id;
 ...
 const h=el('h3'),a=el('a',e.title);a.href='#'+e.id;h.append(a);c.append(h,el('p',e.summary)); if(e.president)c.append(paragraph('Presidente no marco:',e.president)); if(e.authority)c.append(paragraph('Autoridade / ato:',e.authority));
...
function render(updateURL=true,keepPage=false){
 ...
 let filtered=filterEvents(events,state()); const s=state(); if(s.impact)filtered=filtered.filter(e=>e.impact===s.impact); if(s.president)filtered=filtered.filter(e=>(e.president||'')===s.president); if(s.authority)filtered=filtered.filter(e=>(e.authority||'')===s.authority); const out=$('timeline');out.replaceChildren();
...
function reset(){for(const k of ['q','sphere','status','year','impact','president','authority'])$(k).value='';$('order').value='desc';topic='';render();}
$('more').onclick=()=>{visibleCount+=12;render(false,true);};
$('filters').onsubmit=e=>e.preventDefault();$('q').oninput=()=>render();for(const k of ['sphere','status','order','year','impact','president','authority'])$(k).onchange=()=>render();$('reset').onclick=reset;
...
async function loadEvents(){
 ...
 $('year').replaceChildren(new Option('Todos os anos',''));for(const year of [...new Set(events.map(e=>e.year))].sort((a,b)=>b-a))$('year').add(new Option(String(year),String(year))); $('impact').replaceChildren(new Option('Todos','')); for(const v of [...new Set(events.map(e=>e.impact).filter(Boolean))].sort())$('impact').add(new Option(v,v)); $('president').replaceChildren(new Option('Todos','')); for(const v of [...new Set(events.map(e=>e.president).filter(Boolean))].sort())$('president').add(new Option(v,v)); $('authority').replaceChildren(new Option('Todas','')); for(const v of [...new Set(events.map(e=>e.authority).filter(Boolean))].sort())$('authority').add(new Option(v,v)); $('coverage').textContent=events.length+' registros conferidos • Revisão: '+date(data.reviewedAt)+' • '+data.coverage;
